...
export class CompartidoService {
  ...
  private async assertMiembro(asociacionId: string, userId: number) {
    ...
    if (!asoc) throw new NotFoundException('Association not found');
    if (asoc.ownerId !== userId && asoc.partnerUserId !== userId) throw new ForbiddenException('Not authorized');
    return asoc;
  }
  ...
  private async ensureFondoCompartido(userId: number) {
    ...
    const created = await this.prisma.fondoInversion.create({
      data: { userId, nombre: 'Shared', monto: 0, descripcion: 'Auto-generated fund for shared transactions' },
    });
    return created.id;
  }
  ...
  async invitar(ownerId: number, dto: {
    ...
    if (dup) throw new BadRequestException('An invitation to that email already exists');
  ...
  async aceptarInvitacion(partnerUserId: number, asociacionId: string, aliasParaPartner?: string) {
    const asoc = await this.prisma.asociacion.findUnique({ where: { id: asociacionId } });
    if (!asoc) throw new NotFoundException('Association not found');
    if (asoc.estado !== AsociacionEstado.PENDIENTE) throw new BadRequestException('Invitation is not pending');
    if (asoc.partnerUserId) throw new BadRequestException('Invitation already accepted');
  ...
  // ===== Permissions & Ocultos =====
  async obtenerPermissions(asociacionId: string, userId: number) {
  ...
  async dashboard(asociacionId: string, requesterId: number, range?: { from?: Date; to?: Date }) {
    ...
    if (!asoc) throw new NotFoundException('Association not found');
    if (asoc.estado !== AsociacionEstado.ACTIVA) throw new BadRequestException('Association not active');
    if (![asoc.ownerId, asoc.partnerUserId].includes(requesterId)) throw new ForbiddenException('Not authorized');
  ...
  async crearMovimientoCompartido(asociacionId: string, requesterId: number, dto: {
    ...
    if (!partnerId) throw new BadRequestException('Association has no accepted partner');
    ...
    if (Number((aporteOwner + aportePartner).toFixed(2)) !== Number(total.toFixed(2))) {
      throw new BadRequestException('Contributions do not add up to the total');
    }
    ...
    return this.prisma.$transaction(async tx => {
      ...
      if (dto.modulo === ModuloType.INGRESOS) {
        await tx.ingreso.createMany({
          data: [
            ...
            { userId: partnerId, fuente: dto.concepto, monto: aportePartner, frecuencia: 'único', fecha, fijo: false, categoria: 'Shared', isShared: true, sharedId: mov.id, colorTag: color },
          ]
        });
      }
      ...
      if (dto.modulo === ModuloType.GASTOS) {
        ...
        await tx.gasto.createMany({
          data: [
            { userId: ownerId,   descripcion: dto.concepto, monto: aporteOwner,  origen: 'Shared', fecha, fijo: false, frecuencia: 'único', categoriaId, isShared: true, sharedId: mov.id, colorTag: color },
            { userId: partnerId, descripcion: dto.concepto, monto: aportePartner, origen: 'Shared', fecha, fijo: false, frecuencia: 'único', categoriaId, isShared: true, sharedId: mov.id, colorTag: color },
          ]
        });
      }
